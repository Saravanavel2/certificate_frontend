...
const STEP_META = {
    ...
    hashing: { label: 'Layer 2 · Cryptographic Integrity', icon: 'pi-key', color: '#6366F1', num: 2 },
    parsing: { label: 'Layer 3 · Metadata Extraction', icon: 'pi-database', color: '#0EA5E9', num: 3 },
    registry: { label: 'Layer 4 · Registry Consensus', icon: 'pi-globe', color: '#10B981', num: 4 },
};

const STEP_ORDER = ['qrcode', 'hashing', 'parsing', 'registry'];
...
const VerificationPage = ({ onBack }) => {
    ...
    const handleInstantVerify = async (id) => {
        ...
        setVerifying(true);
        setSteps([
            { id: 'qrcode', status: 'pass', message: 'QR Code URL parsing successful.', comparisons: [{ label: 'Source', got: 'Instant Verification Link', match: true }] },
            { id: 'registry', status: 'working', message: 'Querying official registry by ID...' }
        ]);
    ...
    const onUpload = async (e) => {
        ...
        // Immediately start with Layer 1 (QR Code scan simulation) as passed
        pushStep('qrcode', 'pass', 'QR Code scanned and parsed successfully.', [
            { label: 'QR Status', got: 'Decoded successfully', match: true },
            { label: 'Secure URL', got: 'Verification URL identified', match: true }
        ]);
        esRef.current = es;
            ...
            <nav style={{ maxWidth: 780, margin: '0 auto 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={onBack}>
                    ...
                    <span style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: '1.2rem', color: 'var(--text)' }}>
                        Blockchain <span style={{ color: 'var(--accent)', fontWeight: 400 }}>Validation Portal</span>
                    </span>
                </div>
            ...
            <div style={{ maxWidth: 780, margin: '0 auto' }}>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 28, padding: '40px 32px 32px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--border)', textAlign: 'center', marginBottom: 24 }} data-aos="fade-up">
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', maxWidth: 480, margin: '0 auto 32px', lineHeight: 1.7 }}>
                        This is not a simple link check. Our engine performs a deep, 4-layer cryptographic audit to ensure document integrity and registry consensus.
                    </p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', textAlign: 'left', marginBottom: '32px', padding: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid var(--border)' }}>
                        <div>
                            ...
                            <ul style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', paddingLeft: '18px', margin: 0, lineHeight: 1.6 }}>
                                ...
                                <li>Wait for the 4-layer security engine to finish its audit.</li>
                    ...
                    {!verifying && !result && (
                        <div style={{ border: '2px dashed #CBD5E1', borderRadius: 20, padding: 'clamp(24px, 5vw, 36px)', background: 'rgba(99,102,241,0.02)' }}>
                             <FileUpload mode="basic" name="file" accept="application/pdf" maxFileSize={10000000}
                                customUpload={true} uploadHandler={onUpload} auto chooseLabel="Select Certificate PDF"

...
function CertifyStudio() {
    ...
    const onTemplateUpload = async (e) => {
        ...
        try {
            ...
            toast.error(err.message || err || 'Resolution check failed', {
                icon: '⚠️',
                duration: 5000,
            ...
            if (templateUploadRef.current) {
                templateUploadRef.current.clear();
            }
        }
    };
                ...
                <div className="topbar" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '4px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: 50, border: '1px solid var(--border)' }}>
                            ...
                            <Avatar image={(!user.picture || user.picture === "null" || user.picture === "undefined") ? `https://ui-avatars.com/api/?name=${user.name}&background=3B82F6&color=fff` : user.picture} shape="circle" size="normal" />
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                <div className="upload-zone-premium" style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed var(--border)' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                                        ...
                                        <div>
                                            <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text)' }}>CSV / Excel Data File</div>
                                            <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>{csvData ? `${csvData.participants.length} records loaded` : 'Accepts .csv, .xlsx, .xls — Max 1001 rows'}</div>
                                        </div>
                                    ...
                                    <div style={{ display: 'flex', gap: 10 }}>
                                        <FileUpload ref={csvUploadRef} mode="basic" name="csv" accept=".csv, .xlsx, .xls, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" customUpload={true} uploadHandler={onCsvUpload} auto chooseLabel={csvData ? 'Replace Spreadsheet' : 'Upload Spreadsheet'} className="w-full" />
                                    </div>
                                </div>
                                ...
                                <div className="upload-zone-premium" style={{ background: 'rgba(255,255,255,0.02)', border: '1px dashed var(--border)' }}>
                                    ...
                                    <FileUpload ref={templateUploadRef} mode="basic" name="template" accept="image/jpeg" maxFileSize={2000000} customUpload={true} uploadHandler={onTemplateUpload} auto chooseLabel={templateUrl ? 'Replace Design' : 'Upload Design'} className="w-full" />
                                </div>
            ...
            {isGenerating && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        {/* Real-Time Blockchain Sync Pipeline visual checklist */}
                        {progress?.stage !== 'queued' && (
                            <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 8, background: 'rgba(255,255,255,0.02)', padding: 16, borderRadius: 16, border: '1px solid var(--border)', textAlign: 'left' }}>
                                <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <i className="pi pi-shield" style={{ fontSize: '0.75rem', color: 'var(--accent)' }}></i> Real-Time Blockchain Sync Pipeline
                                </div>
                                {[
                                    { label: "1. Render template & CSV data", limit: 12 },
                                    { label: "2. Generate & embed QR Code", limit: 25 },
                                    { label: "3. Compute SHA-256 integrity hash", limit: 38 },
                                    { label: "4. Inject steganographic metadata", limit: 50 },
                                    { label: "5. Register ledger transaction", limit: 63 },
                                    { label: "6. Broadcast to consortium nodes", limit: 76 },
                                    { label: "7. Network consortium consensus", limit: 88 },
                                    { label: "8. Update distributed immutable ledgers", limit: 100 },
                                ].map((s, idx) => {
                                    const pct = progress?.percent || 0;
                                    const isPassed = pct >= s.limit;
                                    const isCurrent = pct < s.limit && (idx === 0 || pct >= (idx > 0 ? [12, 25, 38, 50, 63, 76, 88][idx - 1] : 0));
                                    return (
                                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.75rem', opacity: isPassed || isCurrent ? 1 : 0.4 }}>
                                            {isPassed ? (
                                                <i className="pi pi-check-circle" style={{ color: '#10B981', fontSize: '0.85rem' }} />
                                            ) : isCurrent ? (
                                                <i className="pi pi-spin pi-spinner" style={{ color: 'var(--accent)', fontSize: '0.85rem' }} />
                                            ) : (
                                                <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', marginLeft: 4, marginRight: 4 }} />
                                            )}
                                            <span style={{ color: isPassed ? '#10B981' : (isCurrent ? 'var(--accent)' : 'var(--text-secondary)'), fontWeight: isCurrent ? 700 : 500 }}>
                                                {s.label}
                                            </span>
                                        </div>
                                    );
